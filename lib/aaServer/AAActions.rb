...
=end
private
    def return_json_response hash, code
...
#
    def version
      return return_json_response({'v' => '1.1.0', 'author' => 'António Almeida'}, 200)
    end
# Test call
#
#
    def test sessions, request, server
      return return_json_response(sessions.sessions, 200)
    end
# Kick user call
...
#
    def kick sessionsObj, request
      ...
      return return_unauthorized 'No Target!' unless kick
      return return_unauthorized 'Session is not valid.' unless sessionsObj.valid_session id, ip
      return return_unauthorized 'No privileges for that action.' unless sessionsObj.mastersSessions.include? ip
      sessionsObj.end_session kick
      return return_json_response({'description' => 'Session cleared.', 'id' => id}, 200)
    end
# Shutdown call
#
#
    def shutdown sessionsObj, request, server
      ...
      return return_unauthorized 'No privileges for that action.' unless sessionsObj.mastersSessions.include? ip
      server.shutdown
      return return_json_response({'status' => 'command sent'}, 200)
    end
# Restart server
#
#
...
#
    def get_pid sessionsObj, request
      id = request.query['id']
      return return_unauthorized 'No Id!' unless id
      return return_unauthorized 'Session is not valid.' unless sessionsObj.valid_session id, request.remote_ip
      process =  Process.pid
      return return_json_response({'Pid' => process}, 200)
    end
...
#
    ...
    def login sessionsObj, request
      id = request.query['id']
      return return_unauthorized 'No Id!' unless id
      return return_unauthorized 'Session already exists.' if sessionsObj.valid_session id, request.remote_ip
      return return_unauthorized 'Max sessions reached.' if sessionsObj.max_session request.remote_ip
...
#
    def logout sessionsObj, request
      id = request.query['id']
      return return_unauthorized 'No Id!' unless id
      return return_unauthorized 'Session does not match.' unless sessionsObj.valid_session id, request.remote_ip
...
=end
    def return_bad_request description = 'Error'
      return return_json_response({'error'=> '400', 'message' => @errorMessages['400'], 'description' => description }, 200)
    end

    def return_unauthorized description = 'Error'
      return return_json_response({'error'=> '401', 'message' => @errorMessages['401'], 'description' => description }, 200)
    end

    def return_method_not_allowed description = 'Error'
      return return_json_response({'error'=> '405', 'message' => @errorMessages['405'], 'description' => description}, 200)
    end
end
# Code by António Almeida
